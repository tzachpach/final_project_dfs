...
results_dir = os.path.join("results", f"ast_{timestamp}")
os.makedirs(results_dir, exist_ok=True)

def build_assists_feature_buckets(columns: pd.Series) -> dict:
    """
    Given df.columns, return categorized feature buckets relevant for predicting assists.
    ...
    for feat in features:
        ...
        if any(x in lower for x in ["ast", "assist", "pass", "creation", "playmaking"]):
            buckets["playmaking"].append(feat)
        if any(x in lower for x in ["ast_to", "ast_ratio", "ast_pct", "usg", "usage", "tov", "ball"]):
            buckets["ball_handling"].append(feat)
        if any(x in lower for x in ["pace", "tempo", "poss", "off_rating", "plus_minus"]):
            buckets["pace_context"].append(feat)
        if any(x in lower for x in ["starter", "venue", "salary", "rest", "position_salary", "fp_", "minutes"]):
            buckets["role_context"].append(feat)
        if any(x in lower for x in ["off_rating", "def_rating", "net_rating", "ast_pct", "usg_pct"]):
...
def add_advanced_transformations(df):
    ...
    skewed_features = ['minutes_played', 'ast', 'usg_pct_x', 'salary-fanduel']
    for col in skewed_features:
        if col in df.columns:
            df[f'{col}_log'] = np.log1p(df[col])
    
    # Polynomial features for key metrics
    key_features = ['minutes_played_rolling_10_day_avg', 'ast_pct_rolling_10_day_avg']
    ...
    for window in windows:
        df[f'ast_ma_{window}'] = df.groupby('player_name')['ast'].transform(
...
feature_buckets = build_assists_feature_buckets(df.columns)
...
print(f"Must-have features: {['minutes_played_rolling_10_day_avg', 'ast_pct_rolling_10_day_avg']}")
...
metrics_df, preds_df, importance_tracker = run_exploration_phase(
    ...
    must_have=["minutes_played_rolling_10_day_avg", "ast_pct_rolling_10_day_avg"],
    n_features=80,
    n_trials=30,
    seed_start=42
)
...
print(f"\n=== Summary for Assists Prediction ===")
