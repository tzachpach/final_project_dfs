...
results_dir = os.path.join("results", f"blk_{timestamp}")
os.makedirs(results_dir, exist_ok=True)

def build_blocks_feature_buckets(columns: pd.Series) -> dict:
    """
    Given df.columns, return categorized feature buckets relevant for predicting blocks.
    ...
    for feat in features:
        ...
        if any(x in lower for x in ["blk", "block", "blka", "rim_protection", "shot_alter"]):
            buckets["shot_blocking"].append(feat)
        if any(x in lower for x in ["paint", "interior", "post", "dreb", "def", "defense"]):
            buckets["interior_defense"].append(feat)
        if any(x in lower for x in ["height", "weight", "size", "wingspan", "minutes", "poss"]):
            buckets["physical_attributes"].append(feat)
        if any(x in lower for x in ["starter", "venue", "salary", "rest", "position_salary", "fp_"]):
            buckets["role_context"].append(feat)
        if any(x in lower for x in ["def_rating", "rating", "plus_minus", "pct_blk", "dreb_pct"]):
...
def add_advanced_transformations(df):
    ...
    skewed_features = ['minutes_played', 'blk', 'dreb', 'def_rating', 'salary-fanduel']
    for col in skewed_features:
        if col in df.columns:
            df[f'{col}_log'] = np.log1p(df[col])
    
    # Polynomial features for key metrics
    key_features = ['minutes_played_rolling_10_day_avg', 'dreb_pct_rolling_10_day_avg']
    ...
    for window in windows:
        df[f'blk_ma_{window}'] = df.groupby('player_name')['blk'].transform(
            lambda x: x.shift(1).rolling(window=window, min_periods=1).mean())
            
    # Trend indicators - with shift(1) to prevent leakage
    df['blk_trend'] = df.groupby('player_name')['blk'].transform(
...
feature_buckets = build_blocks_feature_buckets(df.columns)
...
print(f"Must-have features: {['minutes_played_rolling_10_day_avg', 'dreb_pct_rolling_10_day_avg']}")
...
metrics_df, preds_df, importance_tracker = run_exploration_phase(
    ...
    must_have=["minutes_played_rolling_10_day_avg", "dreb_pct_rolling_10_day_avg"],
    n_features=80,
    n_trials=30,
    seed_start=42
)
