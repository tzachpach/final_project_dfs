...
results_dir = os.path.join("results", f"tov_{timestamp}")
os.makedirs(results_dir, exist_ok=True)

def build_turnovers_feature_buckets(columns: pd.Series) -> dict:
    """
    Given df.columns, return categorized feature buckets relevant for predicting turnovers.
    ...
    for feat in features:
        ...
        if any(x in lower for x in ["tov", "ast", "ast_to", "ast_ratio", "usg", "usage", "ball"]):
            buckets["ball_handling"].append(feat)
        if any(x in lower for x in ["usg_pct", "usage_rate", "poss", "touches", "dribbles"]):
            buckets["usage_context"].append(feat)
        if any(x in lower for x in ["pace", "tempo", "rush", "fast_break", "transition"]):
            buckets["pace_pressure"].append(feat)
        if any(x in lower for x in ["starter", "venue", "salary", "rest", "position_salary", "fp_", "minutes"]):
            buckets["role_context"].append(feat)
        if any(x in lower for x in ["off_rating", "def_rating", "net_rating", "tov_pct", "e_tov_pct", "tm_tov_pct"]):
...
def add_advanced_transformations(df):
    ...
    skewed_features = ['minutes_played', 'tov', 'ast', 'usg_pct_x', 'salary-fanduel']
    ...
    for window in windows:
        df[f'tov_ma_{window}'] = df.groupby('player_name')['tov'].transform(
            lambda x: x.shift(1).rolling(window=window, min_periods=1).mean())
            
    # Trend indicators - with shift(1) to prevent leakage
    df['tov_trend'] = df.groupby('player_name')['tov'].transform(
...
feature_buckets = build_turnovers_feature_buckets(df.columns)
...
print(f"Must-have features: {['minutes_played_rolling_10_day_avg', 'usg_pct_x_rolling_10_day_avg']}")
...
print(f"\n=== Summary for Turnovers Prediction ===")
