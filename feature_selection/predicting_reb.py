...
results_dir = os.path.join("results", f"reb_{timestamp}")
os.makedirs(results_dir, exist_ok=True)

def build_rebounds_feature_buckets(columns: pd.Series) -> dict:
    """
    Given df.columns, return categorized feature buckets relevant for predicting rebounds.
    ...
    for feat in features:
        ...
        if any(x in lower for x in ["reb", "dreb", "oreb", "contested", "box_out"]):
            buckets["rebounding_stats"].append(feat)
        if any(x in lower for x in ["height", "weight", "minutes", "plus_minus", "poss"]):
            buckets["physical_context"].append(feat)
        if any(x in lower for x in ["starter", "venue", "salary", "rest", "position_salary", "fp_", "nba_fantasy_pts"]):
            buckets["role_context"].append(feat)
        if any(x in lower for x in ["off_rating", "def_rating", "net_rating", "pace", "reb_pct", "dreb_pct", "oreb_pct"]):
...
def add_advanced_transformations(df):
    ...
    skewed_features = ['minutes_played', 'reb', 'dreb', 'oreb', 'salary-fanduel']
    for col in skewed_features:
        if col in df.columns:
            df[f'{col}_log'] = np.log1p(df[col])
    
    # Polynomial features for key metrics
    key_features = ['minutes_played_rolling_10_day_avg', 'reb_pct_x_rolling_10_day_avg']
    ...
    for window in windows:
        df[f'reb_ma_{window}'] = df.groupby('player_name')['reb'].transform(
            lambda x: x.shift(1).rolling(window=window, min_periods=1).mean())
            
    # Trend indicators - with shift(1) to prevent leakage
    df['reb_trend'] = df.groupby('player_name')['reb'].transform(
...
def run_rolling_xgb_training(df: pd.DataFrame, config: Dict) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    # Cast categorical columns at the DataFrame level
    for col in cat_cols:
        if col in df.columns:
    ...
    numeric_features = [f for f in all_feats if f not in cat_cols]
    ...
    df_for_training[numeric_features] = scaler.fit_transform(df_for_training[numeric_features])
    ...
    for test_idx in range(lookback, len(weeks)):
...
feature_buckets = build_rebounds_feature_buckets(df.columns)
...
config_template = {
    "target_col": "reb",  # Changed from pts to reb
    "categorical_features": categorical_features,
    "numeric_features": [],  # overridden per trial
    "salary_quantile_threshold": 0.75,  # threshold for training data
    "metrics_salary_quantile": 0.9,     # threshold for calculating metrics
...
print(f"Must-have features: {['minutes_played_rolling_10_day_avg', 'reb_pct_rolling_10_day_avg']}")
...
metrics_df, preds_df, importance_tracker = run_exploration_phase(
    ...
    must_have=["minutes_played_rolling_10_day_avg", "reb_pct_rolling_10_day_avg"],
    n_features=80,
    n_trials=30,
    seed_start=42
)
...
print(f"\n=== Summary for Rebounds Prediction ===")
...
# Analyze results
print("\n=== Feature Coverage in Exploitation Phase ===")
for feat, count in sorted(coverage.items(), key=lambda x: x[1], reverse=True)[:10]:
    print(f"{feat}: used in {count} trials")
