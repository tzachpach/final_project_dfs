...
def predict_fp_xgb(
    ...
    if xgb_param_list is None:
        xgb_param_list = [
            {}
        ]  # means no custom param override (just uses default inside rolling_train_test_for_xgb)
    ...
    if start_date:
        df_filtered = df_filtered[
            df_filtered["game_date"] >= pd.to_datetime(start_date)
        ]
    if end_date:
        df_filtered = df_filtered[df_filtered["game_date"] <= pd.to_datetime(end_date)]
    if df_filtered.empty:
        raise ValueError(
            "No data after applying year/date filters. Please adjust filters."
        )

    # Check if we have multiple combos to run
    do_multi_run = (
        len(train_window_days_list) > 1
        or len(xgb_param_list) > 1
        or len(percentile_list) > 1
    )
