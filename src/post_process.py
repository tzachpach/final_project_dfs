...
def post_process_results(df):
    ...
    df["is_repeat_selected"] = df["lineup_selected"].apply(
        lambda x: len(x) - len(np.unique(x))
    )
    ...
    df["budget_efficiency_selected"] = df.apply(
        lambda row: calculate_kpis(row)[4], axis=1
    )
    ...
    avg_f1 = df["f1"].mean()
    avg_budget_efficiency_gain = (
        df["budget_efficiency_selected"].mean() / df["budget_efficiency_ideal"].mean()
    )
    # avg_top_player_accuracy = df['top_player_accuracy'].mean()
    # avg_positional_accuracy = df['positional_accuracy'].mean()

    aggregate_res = pd.DataFrame(
        {
...
def calculate_kpis(row):
    ...
    budget_efficiency_ideal = (
        row["actual_score_ideal"] / row["cost_ideal"] if row["cost_ideal"] > 0 else 0
    )
    budget_efficiency_selected = (
        row["actual_score_selected"] / row["cost_selected"]
        if row["cost_selected"] > 0
        else 0
    )
    ...
    return (
        ...
        budget_efficiency_ideal,
        budget_efficiency_selected,
    )  # top_player_accuracy, positional_accuracy
