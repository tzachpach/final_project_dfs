...
def data_loader(n=10):
    ...
    res_df["is_reg_season"] = res_df["date"].apply(
        lambda x: x.month in [10, 11, 12, 1, 2, 3, 4]
    )
    res_df = res_df[res_df["is_reg_season"]]
    ...
    print(
        f"Dropped {len(res_df_no_mv) - len(res_df_no_mv_over_10_mins)} players with under {n} minutes"
    )
    ...
    for num_prev_games in num_previous_games:
        for column in columns_to_roll:
            rolling_features.update(
                {
                    f"{column}_prev_{num_prev_games}": res_df_no_mv_over_10_mins.groupby(
                        "player"
                    )[
                        column
                    ]
                    .rolling(window=num_prev_games, min_periods=1)
                    .mean()
    ...
    res_df_no_mv_over_10_mins = pd.concat(
        [res_df_no_mv_over_10_mins, rolling_features_df], axis=1
    )
    train_data = res_df_no_mv_over_10_mins[
        [col for col in res_df_no_mv_over_10_mins.columns if col not in columns_to_roll]
    ]
...
def get_game_by_game_data(season: str):
    ...
    for game_id in game_ids:
        ...
        cols_to_use = advanced_df.columns.difference(
            traditional_df.columns
        ).tolist() + ["PLAYER_ID"]
        merged_df = pd.merge(
            traditional_df, advanced_df[cols_to_use], on="PLAYER_ID", how="left"
        )

        game_date = games_df.loc[games_df["GAME_ID"] == game_id, "GAME_DATE"].iloc[
            0
        ]  # Fetching first value assuming it's unique
    ...
    relevant_teams = (
        all_game_data_df.groupby("TEAM_ABBREVIATION")
        .size()
        .sort_values(ascending=False)
        .head(30)
        .index
    )
    all_game_data_df = all_game_data_df[
        all_game_data_df["TEAM_ABBREVIATION"].isin(relevant_teams)
    ]
