...
def get_best_match(name, name_list, threshold=80):
    best_match, best_score = process.extractOne(
        name, name_list, scorer=fuzz.token_set_ratio
    )
...
def get_lineup(df):
    ...
    for date in unique_dates:
        ...
        for platform in ["fanduel"]:  # , 'yahoo', 'draftkings'
            ...
            df_filtered_pred, best_individual_pred = get_best_lineup(
                date, df, platform, pred_flag=True
            )
            ...
            spec_date.update(
                {
                    ...
                    f"{platform}_predicted_players": [
                        df_filtered_pred.iloc[i]["player_name"]
                        for i in best_individual_pred
                    ],
            ...
            # ----- Overlap information -----
            overlap_indices = set(best_individual).intersection(
                set(best_individual_pred)
            )
            overlap_player_names = [
                df_filtered.iloc[i]["player_name"] for i in overlap_indices
            ]
...
def get_best_lineup(date, df, platform="fanduel", pred_flag=False, use_ilp=False):
    ...
    toolbox.register(
        "attr_unique", random.sample, range(len(df_filtered)), num_players_selected
    )
    toolbox.register(
        "individual", tools.initIterate, creator.Individual, toolbox.attr_unique
    )
    toolbox.register("population", tools.initRepeat, list, toolbox.individual)

    def evaluate(individual):
        total_cost = sum(
            df_filtered.iloc[i][f"{platform.lower()}_salary"] for i in individual
        )
        penalty = max(0, total_cost - salary_cap) * 10
        pred_col = (
            f"fp_{platform.lower()}_pred" if pred_flag else f"fp_{platform.lower()}"
        )
    ...
    try:
        ...
        return df_filtered, []
    algorithms.eaSimple(
        population, toolbox, cxpb=0.5, mutpb=0.2, ngen=10, verbose=False
    )
