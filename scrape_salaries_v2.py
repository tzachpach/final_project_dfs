...
def merge_dfs(dfs_dict):
    ...
    merged_df = reduce(
        lambda left, right: pd.merge(left, right, on=common_columns, how="outer"),
        dfs_dict.values(),
    )
    ...
    for date in new_dates:
        ...
        for game in games:
            ...
            try:
                ...
                salary_df = salary_df[
                    salary_df[f"{game}_salary"].str.contains("^\$", regex=True)
                ]
                ...
                salary_df[f"{game}_salary"] = (
                    salary_df[f"{game}_salary"]
                    .str.replace("$", "")
                    .str.replace(",", "")
                    .astype(float)
                )
                salary_df["player_name"] = (
                    salary_df["player_name"].str.split(", ").str[::-1].str.join(" ")
                )
        ...
        existing_dfs_data = pd.concat(
            [existing_dfs_data, merged_df[end_cols]], ignore_index=True
        ).drop_duplicates()
        existing_dfs_data = (
            existing_dfs_data.sort_values("date")
            .drop_duplicates(["date", "player_name"])
            .reset_index(drop=True)
        )

        print(f"Processed date: {date}, Num results: {len(merged_df)}")

    existing_dfs_data = existing_dfs_data.sort_values(
        "date", ascending=False
    ).reset_index(drop=True)
    existing_dfs_data.to_csv(
        f"output_csv/salaries_data_{start_date}-{end_date}.csv", index=False
    )
